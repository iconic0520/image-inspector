"""
Image Inspector — "Forensic Gallery" Edition
═══════════════════════════════════════════════
Design system translated from Google Stitch template.
Primary: #005DAC  |  Surface hierarchy: #F8F9FA → #FFFFFF
Font: Inter (via tkinter fallback chain)

Dependencies:
    pip install pillow numpy opencv-python imagehash tkinterdnd2 psutil
"""
...
# ══════════════════════════════════════════════
#  FORENSIC GALLERY DESIGN SYSTEM
# ══════════════════════════════════════════════

# Surface hierarchy (light → lifted)
BG          = "#F8F9FA"   # App canvas
SURF_LOW    = "#F3F4F5"   # Sidebar / nav
SURF        = "#EDEEEF"   # Containers
SURF_HIGH   = "#E7E8E9"   # Inset / log areas
SURF_HIGEST = "#E1E3E4"   # Progress track / tags
SURF_LOWEST = "#FFFFFF"   # Main cards (lifted)
...
# Text
ON_SURF     = "#191C1D"   # Primary text
ON_SURF_VAR = "#414752"   # Secondary / muted
OUTLINE     = "#717783"   # Tertiary / hints
OUTLINE_VAR = "#C1C6D4"   # Ghost borders
...
def analyse_tampering(img_path):
    r = {}
...
class App(AppBase):
    ...
    def _stop_task(self):
        """Signal any running background thread to stop."""
        self._stop_flag.set()
    ...
    def _do_single(self, path):
        ...
        if self._stop_flag.is_set():
            self._t1_log("[STOPPED] Analysis cancelled by user.\n")
            self._task_done(); return
        try: r = analyse_tampering(path)
        except Exception as e: self._t1_log(f"ERROR: {e}\n"); self._task_done(); return
    ...
    def _do_batch(self, folder):
        ...
        for i,fpath in enumerate(files):
            ...
            if self._stop_flag.is_set():
                self._t1_log(f"[STOPPED] Batch cancelled after {i} images.\n")
                self._task_done(); return
            try: r = analyse_tampering(fpath)
    ...
    def _do_t2(self, src, good_dir, bad_dir, opts, mode):
        ...
        for i,fpath in enumerate(files):
            ...
            if self._stop_flag.is_set():
                self._t2_log(f"[STOPPED] Sort cancelled after {i} files.\n")
                self._task_done(); return
            reasons=[]
